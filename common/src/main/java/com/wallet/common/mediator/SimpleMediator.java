...
public class SimpleMediator implements Mediator {
    ...
    @Override
    public <C, R> R send(C commandOrQuery) {
        ...
        for (CommandHandler handler : commandHandlers.values()) {
            ...
            if (handledType != null && handledType.isAssignableFrom(commandOrQuery.getClass())) {
                return (R) handler.handle(commandOrQuery);
            }
        }
        ...
        throw new IllegalArgumentException("No suitable handler found for " + commandOrQuery.getClass().getName());
    }
